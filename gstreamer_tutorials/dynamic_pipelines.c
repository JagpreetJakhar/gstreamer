...
int main(int argc, char *argv[]) {
  ...
  if (!data.pipeline || !data.source || !data.aconvert ||!data.vconvert || !data.resample || !data.asink ||!data.vsink) {
  ...
  gst_bin_add_many (GST_BIN (data.pipeline), data.source, data.aconvert,data.vconvert, data.resample, data.asink,data.vsink, NULL);
...
g_print("All Audio and Video elements created and linked sucessfully\n");
...
static void pad_added_handler (GstElement *src, GstPad *new_pad, CustomData *data) {
  ...
  if (gst_pad_is_linked (asink_pad) && gst_pad_is_linked(vsink_pad)) {
    g_print ("Both audio and video are already linked. Ignoring.\n");
    goto exit;
  }
  ...
  if (GST_PAD_LINK_FAILED (ret)) {
    g_print ("AudioLink: Type is '%s' but link failed.\n", new_pad_type);
  } else {
    g_print ("AudioLink:Link succeeded (type '%s').\n", new_pad_type);
  }}
